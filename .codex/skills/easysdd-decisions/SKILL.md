...
description: easysdd-decisions 是 easysdd 的决策归档子工作流——将项目中已经拍板的技术选型、架构决定、长期约束和编码规约记录成可检索的决策文档，让任何人（包括未来的 AI）都能在几秒内找到"我们为什么用 X"或"这里为什么不能用 Y"的答案。当用户说"记录一个决定"、"归档技术选型"、"我们定下来用 X 了"、"为什么用 X 而不是 Y"、"记录这个约束"、"我们的规约是"、"decisions"、"ADR"、"decision record"、"把这条规约写下来"时触发。如果在 easysdd-feature-design 或 easysdd-issue-analyze 阶段做出了对项目长期有影响的重要选择，AI 应主动询问"要把这个决定记录下来吗？"——用户说"不用"立即跳过。
---
