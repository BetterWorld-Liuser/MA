...
description: 按已经写好的方案 doc(含不变量清单)实现一个新功能,走完阶段四。当用户说"方案确认了,开始实现"、"按方案写代码"、"开工"、"开始 coding"、"实现这个功能"等意图,且方案 doc 已经存在并完整(含第 7 节不变量)时,务必触发本技能。本技能严格按方案的"推进顺序"执行,一次性完成所有代码,完成后输出统一的完成汇报,等用户 review 通过再进入验收阶段。本技能是 easysdd-feature 工作流的第四阶段,前置依赖 easysdd-feature-design + easysdd-feature-test-spec 的产出。如果方案 doc 不存在或第 7 节不变量空缺,先引导用户走前置技能。
---
