...
description: 在一个还没有 easysdd 目录结构的代码仓里，或只有零散文档的仓里，构建起 easysdd 的完整目录体系。当用户说"我想在这个项目里用 easysdd"、"帮我搭一下 easysdd 目录结构"、"把这个项目接入 easysdd"、"初始化 easysdd"、"这个仓库没有 easysdd 结构"、"迁移到 easysdd"、"帮我整理一下已有文档"时触发。本技能分两条路径：绿地路径（仓库内无任何相关文档结构，从零创建）和迁移路径（已有零散文档、部分目录或半成品 easysdd 结构，需审计后归位）。产出：符合 easysdd 主技能第二节规范的完整 easysdd/ 目录骨架，以及已有文档的迁移方案（迁移路径），由用户逐一确认后落盘。
---
...
- **中**：可以推断但有歧义（比如一份文档看起来同时包含 PRD 和设计方案）
