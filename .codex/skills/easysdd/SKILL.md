---
name: easysdd
description: easysdd(Easy Spec-Driven Development)是本项目的规约驱动开发体系——所有非 trivial 的工作都先产出 spec(PRD / 方案 doc / 不变量),再写代码,最后做闭环验收。当用户提到 "easysdd"、"sdd"、"规约驱动"、"我想了解这套工作流"、"怎么用这个流程"、"我现在该用哪个技能"、"从哪开始"、"介绍一下 easysdd",或者用户描述了一个新功能但还没决定从哪个阶段切入时,务必触发本技能。本技能是 easysdd 家族的根/中心,负责:(1) 介绍工作流的"为什么"和分阶段结构;(2) 根据用户当前已有的产物把他路由到正确的子技能;(3) 沉淀跨阶段的共同原则;(4) 作为未来扩展(新工作流、新约束、新模板)的挂载点。如果用户已经明确说"开始写 PRD / 方案 / 实现 / 验收"等具体阶段意图,优先触发对应的子技能而不是本技能。
---
...
**本节是整个 easysdd 家族目录约定的唯一定义处**。本技能下文以及所有子技能、工作流文档在正文里都**用自然语言术语**(PRD 文档、方案 doc、feature 目录……)引用产物,它们的字面位置看下面这棵目录树就能找到。目录结构要改,只在本节改一次。

### easysdd 根目录下有什么

`easysdd/` 下有五个子目录,每个的用途都固定:

- **`architecture/`** —— 项目级架构权威目录(AGENTS.md 钦定的"架构中心"),长期存在、跨 feature 共享。里面有 `DESIGN.md`(架构总入口),以及各子系统架构 doc。
- **`features/`** —— 所有 feature spec 的聚合根。每个 feature 一个子目录 `{feature}/`,里面住着该 feature 的 `brainstorm.md`(Stage 0 brainstorm note,可选)、`prd.md`(PRD 文档)、`design.md`(方案 doc)、`acceptance.md`(验收报告)。同一 feature 的所有 spec 产物(包括可选的 brainstorm note)永远聚合在一起。
...
>
> - **日期前缀**(`YYYY-MM-DD`)—— 取该 feature 目录**首次创建**当日的日期,一经确定就不变。前缀的作用是让 `features/` 下的子目录天然按时间排序,方便后续翻阅。哪怕 Stage 1 PRD 把 feature 改名了,**日期前缀也不改**,只替换后半段的 slug。
...
1. **一个 feature = 一个 feature 目录**。同一 feature 的 brainstorm / prd / design / acceptance 永远聚合在一起;
...
5. **learning 文档单独归档**。`learnings/` 是累积性知识库,只放 easysdd-compound 产出的坑点/知识文档;spec 产物(prd/design/acceptance/report)不放进来,learning 文档也不放进 `features/` 或 `issues/`。
...
7. **工具脚本统一放 `tools/`**。跨工作流共享的脚本都放在 `tools/` 下,不要散落在项目根目录或子工作流目录里。新增工具时同步在本节目录树登记。
8. **Stage 0 brainstorm 也归属 feature 目录**。brainstorm note 就住在它所属 feature 的目录下(`brainstorm.md`),和 prd/design/acceptance 聚合在一起——同一 feature 的所有 spec 产物一律同址。由于 brainstorm 开始时 feature slug 可能还没最终敲定,做法是:和用户商定一个**临时 slug**,按上文 `YYYY-MM-DD-{英文 slug}` 的格式拼出 feature 目录名(日期就取开 brainstorm 当天),然后以此创建 feature 目录。如果 Stage 1 PRD 把后半段 slug 改了,**只改 slug、不改日期前缀**,连同 feature 目录一起重命名,brainstorm note 跟着走。严禁再往 `easysdd/brainstorms/` 这种并列目录塞文件。
9. **新加子工作流 / 新产物类型**,默认在 `easysdd/` 下开新子目录,并在本节登记。不要往别处塞新东西,也不要只在子技能里加路径而不回中心登记。
10. **路径变更唯一源**:要改目录结构,先改本节的目录树和说明,其他地方自动跟随。

---

## 三、目前包含的子工作流

### easysdd-feature(新功能开发,可选 Stage 0 + 五个正式阶段)
...
| ③ 测试设计 | `easysdd-feature-test-spec` | AI 起草,用户 review | 回填到方案 doc 第 7 节 |
...
**底层入口**:`easysdd-feature` 子技能(含路由表、Stage 0 判断逻辑、目录规则)。**底层权威源**:`workflows/需求实现工作流.md`。
...
| ③ 修复验证 | `easysdd-issue-fix` | AI 定点修复,用户验证 | 代码修复 + 修复记录(`fix-note.md`) |
...
- 目录结构和路径变量见第二节"目录安排",本节不重复。两条最容易出错的原则反复强调:
  - **一个 feature = 一个 feature 目录**——同一 feature 的 prd / design / acceptance 永远聚合在一起
...
- 撞了:要么改新概念名,要么复用既有命名
- 这条规则的执行强度从阶段二开始,但即使在阶段一(PRD)阶段,也要避免引入将来在阶段二会被否的术语
...
### 6. UI 改动必须浏览器验证
- 涉及前端视觉/交互的不变量,自动化测试不够,必须在阶段五人工肉眼验证
- 这是 `AGENTS.md` 的硬要求
...
2. 问"你之前写过 PRD / 方案 doc 吗?"或者直接 Glob 所有 feature 目录下的 brainstorm note / PRD / 方案 doc,看有没有相关文件
3. 按第四节路由表把用户导到对应子技能
...
- `workflows/需求实现工作流.md` — easysdd-feature 的权威源(本中心技能不重复其内容)
