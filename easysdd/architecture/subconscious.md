...
构建 Subconscious 的 API 请求时，turn_history 中的 thinking block（Anthropic signature block 等）自动剥离，不透传。Subconscious 仍能看到完整的工具调用、工具结果和 assistant 文本回复，只是不包含推理过程内容。

这样处理的原因：Subconscious 关心的是"主 agent 做了什么"，而不是"主 agent 怎么想的"；且剥离后 turn_history 对任何 provider 都合法，Subconscious 的模型不需要与主 agent 相同。
...
| 执行命令（脚本型） | **允许** | Script Subconscious 专属；只读命令，不写文件，输出注入为 hint |
| 写入文件 | **禁止** | 后台进程不应产生文件系统副作用 |
...
Subconscious 分为两种子类型，定义文件中是否包含 `run` 字段决定类型：

| 子类型 | 标志 | LLM | 典型用途 |
|--------|------|-----|---------|
| **Script Subconscious** | `run` 字段 | 无 | codecheck、lint、测试、脚本统计 |
| **AI Subconscious** | 正文 prompt | 有 | 记忆整理、模式识别、主动提醒 |

两者共用同一管道、同一 `order` 排序、同一 hint 注入机制。

---

## AI + 程序混合执行

### Script Subconscious 执行流
...
触发
  ...
  │
  ├─ 检查退出码与 on 条件
  │   ├─ on: failure 且退出码为 0 → 跳过，不注入
  │   └─ 需要注入 → 截断输出到 token 上限，写入 hint
  │
  └─ 命令本身出错（超时、不存在等）→ 注入错误 hint，不静默失败
```
...
trigger: turn_end
frequency: 3
model: claude-haiku-4-5   # 可选，省略时使用全局 subconscious 默认模型
pre_check:
...
| 字段 | 必填 | 适用子类型 | 说明 |
|------|------|-----------|------|
| `name` | 否 | 两者 | 省略时用文件名（不含 `.md`） |
| `order` | 否 | 两者 | 执行优先级，数字升序，默认 500 |
| `trigger` | 是 | 两者 | 触发时机（见下方） |
| `frequency` | 否 | 两者 | 每 N 次触发才真正执行一次，省略 = 每次执行；计数器 per-session，session 启动时归零 |
| `model` | 否 | AI | 指定本条 subconscious 使用的模型；省略时使用全局 subconscious 默认模型（见下方），再 fallback 到当前 Task 的活跃模型 |
| `run` | 否* | Script | 要执行的命令；有此字段即为 Script 子类型 |
| `on` | 否 | Script | `always`（默认）或 `failure`（仅非零退出码时注入 hint） |
| `hint_ttl` | 否 | Script | hint 存活轮数，默认 1 |
| `pre_check` | 否 | AI | 程序阶段条件列表，全部满足才进入 AI 阶段 |

**全局 Subconscious 默认模型**：设置页"后台"区域提供一个"默认模型"选项，适用于所有未指定 `model` 字段的 AI Subconscious。未配置时 fallback 到当前 Task 的活跃模型。Script Subconscious 无 LLM 调用，`model` 字段对其无意义，忽略。

`run` 与正文 prompt 互斥：有 `run` 字段则为 Script 型，正文被忽略；无 `run` 字段则为 AI 型，正文即 `subconscious_prompt`。

Script 型输出超过 token 上限时（默认 500 tokens）从尾部截断，并附注 `[output truncated]`。

AI 型 March 在注入时会自动追加工具限制声明（"以下工具已禁用：run_command、write_file、open_file、close_file、write_note、remove_note……调用将返回错误"）。

### 错误处理

**AI Subconscious**：LLM 调用失败（超时、API 错误、rate limit 等）时静默跳过，不阻塞主 agent 也不向用户展示；错误写入本地日志，dev 模式下可在诊断面板查看。不做自动重试——下次触发条件满足时自然再试。

**Script Subconscious**：命令本身出错（进程不存在、超时等）时注入错误 hint，不静默失败；命令正常执行但 `on: failure` 且退出码为 0 时，跳过注入。

### 项目级与用户级同名冲突

同一 `name` 的 subconscious 定义，**项目级完全替换用户级**，用户级定义静默跳过。语义与 git local-overrides-global 一致。如需在项目级扩展用户级行为而非替换，应使用不同的 `name` 和 `order` 定义为两个独立的 subconscious。
...
**`idle(N)` trigger 的特殊约束**：idle 不在主 agent 轮次结束后触发，此时没有刚完成的轮内历史，`turn_history` 为空列表。因此：

- `pre_check: turn_has_tool_calls: true` 永远不满足 → 自动跳过，零成本
- 依赖 turn_history 分析的 AI Subconscious **不应使用** idle trigger
- idle 适合不依赖轮内历史的任务：定期统计、staleness 检查、只读脚本（Script Subconscious 不受影响）
