...
CREATE TABLE turn_snapshots (
    turn_id      TEXT    PRIMARY KEY,  -- 后端分配的字符串 turn_id，对应 Turn.turn_id
...
CREATE TABLE turn_changed_files (
    turn_id      TEXT    NOT NULL REFERENCES turn_snapshots(turn_id),
    path         TEXT    NOT NULL,
...
Turn 的 `turn_id` 是字符串，由后端在 `turn_started` 事件中分配，与 `TaskTimelineEntry[]` 中 `Turn.turn_id` 一一对应。
...
turn 结束、有文件变更时，在 `<TurnGroup />` 卡片底部追加一个折叠区域（位于最终消息展开内容的下方）：
...
- "撤销此轮"按钮触发回退流程（见下节）
- 此区域随 Turn 持久化数据加载，重新打开应用后仍可查看历史变更
