...
其他 coding agent 常见的问题，不是记不住，而是越聊越脏。读文件、改文件、跑命令留下的历史记录一路累积，久了以后，AI 面前会同时摆着很多已经过期、彼此打架的旧快照。

**March 的卖点是：上下文不会腐烂。** 它不靠一直累积旧上下文来维持能力，而是每一轮都尽量回到当前项目状态，让 AI 始终面对一份干净的工作台。

这通过两个设计实现：

1. **文件系统映射**：用 file watcher 监控工作目录，把“磁盘上现在是什么样”持续映射成 AI 当前看到的上下文。无论是 AI 改的、用户手动改的、还是其他程序改的，March 关心的都不是“历史上改过什么”，而是“此刻真实状态是什么”
2. **上下文每轮重建，不线性累积**：大小由当前状态决定，而不是随对话轮次增长

这里说的“文件系统映射”，强调的不是文件系统本身，而是上下文管理方式：**AI 看到的是当前项目状态的映射，不是一路累积下来的历史残留。**
...
- `recent_chat`：只保留最近 N 轮人类↔AI 对话（默认 10 轮，可在设置页全局配置，合理范围 5–50），并附带每条消息时间，旧的自动丢弃；若需跨轮保留信息应主动写入 Notes 或 Memory，而不是靠扩大 N
...
- 一条用户消息可同时激活多个 agent，各自作为独立 Turn 并发执行；每个 Turn 内部由同一个 agent 执行到底，轮内不换模型。用户侧输入框不因 Turn 正在运行而锁死，继续编辑下一条消息，发送时再基于当下最新的 chat 快照与 task 运行上下文创建新一轮

这样做的原因是：

- 工作目录和模型选择一样，都是“这个任务要在什么环境里工作”的任务级决策
- 同一应用窗口里，用户可能并行处理多个项目或同一仓库下的不同子目录
- 只有把工作目录下沉到 task，这套“文件系统映射”机制才能稳定落在“当前 task 实际面对的磁盘范围”上

### Session 与 Task 的边界

**Task** 是持久化实体：有 working_directory、model 选择、recent_chat、notes、open_files，存储在 DB，跨 App 启动保留。

**Session** 是 Task 的一次运行时实例：App 加载该 Task 到关闭/切换之间的内存状态。同一 Task 在不同时间的多次使用对应多个 Session。

实际影响：

- `recent_chat` 随 Task 持久化，跨 session 保留——用户重新打开 App 能看到完整历史
- Subconscious 的 `frequency` 计数器 per-session 归零——App 每次启动或 Task 重新激活时重置
- `session_start` / `session_end` trigger 的语义是”Task 在当前 App 生命周期内首次激活/最后离开”，不是”Task 创建/删除”
- 设置页展示 Task 信息时，用”上次活跃时间”而非”session 启动时间”，避免用户感知 session 概念

### open_files 上下文压力管理

open_files 只靠 AI 主动 `close_file` 收缩，AI 忘了关文件会导致上下文悄悄增大。March 通过两级机制兜底：

**第一级（软提示，默认启用）**：当 open_files 占用超过当前模型可用 token budget 的 60% 时，在 `[hints]` 层注入一条 TTL=1 的提示，建议 AI 关闭不再需要的文件。AI 自主决定是否清理，不强制。

**第二级（自动驱逐，默认关闭）**：用户在设置中启用 `auto_evict: true` 后，每轮 context 构建时，对 open_files 中超过 N 轮（默认 5 轮）未被任何 tool call 访问的文件自动 `close_file`（locked 文件豁免）。右栏同步展示”已自动关闭：X 个文件”。
...
这也要求前端运行态和后端任务事实进一步分层：

- 后端 `snapshot` 是任务事实基线，用于首次加载、切换 task、显式 resync、轮次收敛后的已落盘状态
- 前端运行态（`TaskTimelineEntry[]` 聊天时间线、工具进度、滚动位置、右栏 tab、输入框草稿等）必须按事件增量更新，不能继续作为同一个粗粒度 snapshot 的派生物整体抖动
- 同一块 UI 需要同时消费“持久化事实”和“运行时事件”时，应始终以单一 source of truth 存储，再派生视图；不要把 source 和 view 维护成两份镜像缓存
- 对增量合并对象，必须区分“本次明确清空”与“本次未返回”。像 `runtime`、`debug_trace` 这类运行态快照，若 payload 未提供，语义默认是沿用旧值，而不是清空
...
兼容 `AGENTS.md` 约定，但接入方式遵循 March 自己的“文件系统映射”设计：如果工作目录存在 `AGENTS.md`，就在 session 初始化时自动把它加入 `open_files`，并默认设为 locked。
...
- 命令执行是和外部环境交互
- 文件工具是和 March 自己维护的当前项目状态直接交互

前者保留通用性，后者保证基础文件操作简单、稳定、可控。LSP 工具是第三类：语义层查询，补充“文件系统映射”本身无法直接提供的类型和引用信息。

其中 `run_command` 的流式输出属于运行时显示协议，不是持久化事实本身：

- 若 UI 订阅的是“截至当前的完整输出”，协议和命名必须明确其 snapshot 语义，避免被前端误当 delta 重复拼接
- 长时间命令的流式输出实现必须按增量处理设计，不能每次节流都对完整累积 buffer 做全量 decode / strip / transform
- 子进程正常结束、取消、超时三条路径必须共享同一套收尾逻辑，避免 reader drain / wait / join 的生命周期在不同分支悄悄分叉
...
March 在项目级 `.march/diagnostics/` 下持久化诊断日志，用于问题定位时回看关键过程。它和现有 `.march/debug/` 的职责不同：

- `.march/diagnostics/` 是面向排障的稳定诊断记录，按前后端分文件保存
- `.march/debug/` 是开发期查看 provider/context 细节的 debug 输出，不替代诊断日志
- 诊断日志不等于 `runtime_status`、`debug_trace` 或 UI 事件流；它是独立的项目级持久化证据

第一版保持克制：后端只记录关键链路事件，前端只在显式调试调用时写入，不做默认全局埋点，也不提供专门的日志查看 UI。
...
以下为高层里程碑，细化的模块依赖顺序与阶段目标在 [roadmap.md](roadmap.md) 中维护。
