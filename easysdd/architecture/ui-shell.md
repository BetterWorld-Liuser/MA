...
三栏宽度：中栏始终自适应填满剩余空间；左栏和右栏默认给出稳定初始宽度，但用户可以直接拖拽调整，并在需要时收起成窄 rail，再随时展开。

- 左栏 rail 至少保留“展开任务栏”和设置入口
- 右栏 rail 至少保留“展开上下文栏”
- 拖拽分隔条应保持低存在感，只在 hover / drag 时增强，不把布局控制做成一条吵闹的 UI
...
- **外观**：深色 / 浅色主题切换，立即生效并持久化到本地
- **模型**：模型能力配置，设置默认运行模型
- **供应商**：新增/编辑/删除 provider，填写 name、api_key、base_url，保存后可测试连通性
- **角色**：March 内置角色与自定义 agent 的配置
- **记忆**：全景记忆视图，展示并管理所有记忆（不受当前 session 匹配过滤）

这样做的原因是：

- 主题切换属于应用壳层能力，不能塞进任务区或聊天区临时按钮
- provider 与外观都属于”全局设置”，应该共用一个入口，但在设置页内部按职责分区，避免混成单页长表单
- 右侧上下文面板的记忆区只展示当前 session 命中的记忆子集（AI 视角），无法管理全量记忆；全景视图是其在设置页的管理后台（用户视角），两者职责分开

详见 → [记忆系统 · 全景记忆视图](memory.md#全景记忆视图设置页)
