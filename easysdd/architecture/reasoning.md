...
用户在聊天区看到的 reasoning 折叠块，是 UI 层从流式事件（`assistant_stream_delta { field: 'reasoning' }`）增量接收并写入 `AssistantMessage.reasoning` 字段的——该字段是 `TaskTimelineEntry[]` 数据模型的一部分，与 AI 上下文完全独立，不参与任何 provider API 调用。

**回退一轮时**，对应 Turn 整体从 `TaskTimelineEntry[]` 中移除，reasoning 内容随之消失，不存在消息已撤销但 reasoning 孤立残留的中间态。详见 → [Turn 快照与回退](turn-snapshot.md)。

### 模型切换兼容性

轮间切换模型（在两条消息之间）：`recent_chat` 只含纯文本，任何 provider 都能直接消费，无需任何转换。

轮内切换模型：一个 Turn 由一个 agent 执行到底，Turn 内的所有 LLM 调用（多条 Message）使用同一 agent 绑定的模型，不会在轮内发生模型切换。不同 Turn 可以归属不同 agent，因此使用不同模型，但这是 Turn 间并发，不是 Turn 内切换。
...
- **`reasoning_content` 字段存在时**（DeepSeek 官方 API 等）：streaming chunk 中 `delta.reasoning_content` 非空即 emit `field: 'reasoning'` delta，`delta.content` 非空即 emit `field: 'content'` delta。两者在 chunk 层面互斥，无需额外状态。
- **fallback 到 `<think>` 标签时**：WireAdapter 维护 `in_think: bool` 状态，保留一个不超过 8 字节的尾部 buffer 处理跨 chunk 的标签边界。进入 `<think>` 切换为 reasoning 流（emit `field: 'reasoning'`），出 `</think>` 切回 text 流（emit `field: 'content'`），标签本身不 emit 给 UI。完整 response 接收后，同样从 content 中移除该标签，避免持久化时残留。
...
`assistant_stream_delta` 通过 `field` 枚举区分 reasoning 流和正文流（完整字段定义见 [ui-events.md](ui-events.md)）：

```ts
// field === 'reasoning' 时，前端将 delta 追加到目标 Message 的 reasoning 字段
// field === 'content'   时，前端将 delta 追加到目标 Message timeline 末尾的 TextChunk
assistant_stream_delta { ..., message_id, field: 'reasoning' | 'content' | 'tool_call_arguments', delta }
```

对 `OpenAiHidden`，不产生 `field: 'reasoning'` 的 delta；reasoning token 用量通过 `turn_finished` 的扩展字段携带，在右侧面板的上下文用量区展示：

```ts
// turn_finished 在标准字段之外，可携带可选的 reasoning 用量
turn_finished { task_id, seq, turn_id, reason, error_message?, reasoning_tokens_used?: number }
```
...
> **Wire 层行为说明**：对 `InlineTag` 风格，"禁用"仅为展示层行为。WireAdapter 照常接收响应，reasoning 内容被丢弃，不产生 `field: 'reasoning'` 的 delta 事件，`AssistantMessage.reasoning` 保持为空字符串。Token 消耗不因此减少。如需真正节省 token，应切换到该模型的非 thinking 变体（如 `deepseek-chat`）。浮层中应附一行说明："关闭后思考内容不展示，但 token 消耗可能仍存在。"
