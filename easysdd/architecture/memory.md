...
```rust
struct Memory {
    id: String,                 // 对 AI / UI 暴露的稳定字符串 id；项目级 = 文件名（不含 .md），全局级 = stable_id
    level: MemoryLevel,
    scope: MemoryScope,
...
CREATE TABLE memories (
    id           INTEGER PRIMARY KEY,              -- 仅存储层内部主键，不暴露给 AI / UI
    stable_id    TEXT    NOT NULL UNIQUE,          -- 对外稳定字符串 id，如 "user-style"
...
**为什么保留数字主键**：SQLite 内部仍可用整数主键优化存储和关联，但这只是数据库实现细节。对 AI、工具调用、上下文渲染、设置页管理界面，一律展示和接收 `stable_id`，避免出现 `g:12` 这种无语义的 id，降低重复记忆和误更新概率。
...
CREATE VIRTUAL TABLE memory_fts USING fts5(
    memory_id,      -- 项目级: "p:jwt-token-policy"  全局级: "g:user-style"
    title,
    content,
    tags,
    topic,
...
匹配到 12 条相关记忆，可用 recall_memory(id) 查看详情：
  ...
  g:user-style         [pref]     style    用户不喜欢过度注释，改动处只在不直观时加注释
  ...
```

`p:` 前缀 = 项目级记忆（存在 `.march/memories/` 中），`g:` 前缀 = 全局级记忆（存在用户 DB 中）。

全局级记忆虽然底层存在 SQLite 数字主键，但 `memory_index` 中永远展示 `g:<stable_id>`，让 AI 能直接把它当作 update / recall / forget 的目标 id 使用。
...
// 按需加载记忆详情
recall_memory {
    id: String,                    // 索引中展示的 id，如 "p:jwt-token-policy" 或 "g:user-style"
}
...
上下文面板的记忆区是 **AI 视角**：展示当前轮匹配到的记忆子集，反映"AI 这轮实际收到了哪些记忆"。随着记忆积累，用户需要一个完整的管理界面——不受匹配过滤、不依赖当前 session 状态——来审查、清理和组织所有记忆。

这个界面放在设置页的"记忆"分区（与外观、模型、供应商、角色并列）。

### 布局

```
[记忆]  共 47 条  (project: 38 · global: 9)
                                       [+ 新建记忆]
┌─ 筛选：[全部层级 ▾]  [全部类型 ▾]  [搜索标题/话题…]  ─────────────┐

  topic: auth  (8 条)  ⚠ 建议合并
  ──────────────────────────────────────────────────
  p:jwt-token-policy    [fact]     JWT refresh token 有效期 7 天    ↑5 ×0   [编辑][×]
  p:auth-middleware     [decision] 选了 middleware 方案而非 guard   ↑3 ×1   [编辑][×]
  p:auth-race-cond      [caveat]   并发刷新时存在竞态条件            ↑1 ×7   [编辑][×]
  ···

  topic: testing  (3 条)
  ──────────────────────────────────────────────────
  p:test-migration      [pattern]  测试前先跑 seed_users migration  ↑8 ×0   [编辑][×]
  ···

  topic: style  (1 条)
  ──────────────────────────────────────────────────
  g:user-style          [pref]     用户不喜欢过度注释                ↑4 ×2   [编辑][×]
```

### 组织方式

默认按 **topic** 分组展示，每组显示条目数和合并提示（超过 5 条时标 ⚠）。组内按综合分数降序（与上下文面板匹配逻辑一致），让最有价值的记忆排在前面。

- `↑N`：access_count，表示被 AI recall 了多少次
- `×N`：skip_count，表示出现在索引中但 AI 略过了多少次

skip_count 高的条目意味着记忆质量低或已过时，用视觉弱化（文字颜色变暗）提示用户考虑删除或更新。

### 筛选

| 控件 | 可选值 |
|------|--------|
| 层级 | 全部 / Project（`.march/memories/`）/ Global（用户 DB） |
| 类型 | 全部 / fact / decision / pattern / preference / 其他（自创类型） |
| 搜索 | 对标题、话题、tags 做本地全文匹配，即时过滤，无需后端请求 |

筛选结果保留分组结构，只隐藏不匹配的条目；如果某个 topic 组内所有条目都被过滤掉，则整组折叠。

### 单条记忆展示

每行为紧凑单行，信息密度与上下文面板保持一致风格：

```
[id前缀]  [类型标签]  [话题]  标题文字                ↑访问  ×跳过  [编辑][删除]
```

- `p:` / `g:` 前缀以等宽字体弱化展示，与上下文面板的 id 渲染规则一致
- 类型标签用小圆角背景，与上下文面板保持统一
- 话题大写弱化，标题作为主要阅读焦点
- ↑ / × 统计只在 hover 时展示完整含义标注（`access_count` / `skip_count`），默认仅显示数字

### 编辑与删除

点击"编辑"复用现有 `MemoryEditorDialog`，支持修改类型、话题、标题、内容、tags、scope。  
删除直接执行，无需确认弹窗（可通过 `forget_memory` 撤回——但记忆一旦删除即从存储层移除，不支持撤销）。  

> **与上下文面板的分工**：上下文面板的记忆区不提供"查看全部"或跳转到全景视图的入口，两者职责明确分开——面板是 AI 实时状态的投影，设置页是用户的管理后台。用户若想管理记忆，主动进设置页；若想了解"AI 这轮看到了什么记忆"，看右侧面板。
