...
参考 Claude Code 官网风格：深色背景、低饱和度边框、橙色 accent。桌面 UI 文字默认使用无衬线字体，代码、路径、命令等技术内容再使用等宽字体。

颜色通过 CSS 变量定义在 `:root`，Tailwind theme 直接引用这些变量。这样调色时只改 CSS 变量，Tailwind 工具类自动生效，无需改 config。
