...
[system_status]      ← Ma 写入，AI 只读；当前被固定的文件列表等运行时状态
...
- **时间过期**：注入时指定 `ttl_secs`，转换为绝对时间戳存入数据库。Ma 重启后直接用时间戳判断是否过期，无需额外逻辑。
...
Ma 启动后在本地监听一个 Unix socket（Windows 上为命名管道），外部工具通过它注入 Hints：
...
接口只监听本地，不对外暴露。外部工具（Telegram bot、通知脚本等）与 Ma 部署在同一台机器上，直接调用即可。
...
struct AgentContext {
    ...
    system_status: SystemStatus,                 // Ma 维护，AI 只读
...
数据库文件位于工作目录下的 `.ma/ma.db`，每个工作目录独立一个。
...
**`hints` 跨 task 共享**：外部通知与当前活跃任务无关，所有任务都能看到。Ma 启动时按 `expires_at` 清理已过期条目，运行中按轮次递减 `turns_remaining`。
...
`system_status` 是上下文中 Ma 维护、AI 只读的区域，位于 notes 之下、hints 之上。
...
文件被外部删除、移动或重命名时，Ma 的处理原则：**AI 应感知变化，不静默丢失信息**。
...
Ma 在每轮构建上下文后估算 token 用量。当用量超过阈值时，向 `system_status` 追加压力提示，由 AI 自行决定如何释放空间：
...
AI 收到这条信息后，可以在当前轮或下一轮工具调用中主动整理。Ma 不强制关闭任何内容，决策权留给 AI。压力提示随用量实时更新，用量下降后自动消失。
