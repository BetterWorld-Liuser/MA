...
```
~/.march/skills/git.md          — Git 提交规范
~/.march/skills/rust.md         — Rust 工作流
.march/skills/api-style.md      — 本项目 API 风格约定
```
...
```
1. 用户级  — ~/.march/skills/*.md
2. 项目级  — .march/skills/*.md
```
...
```
可用 Skills：
- rust  (~/.march/skills/rust.md)：Rust 项目工作流
- git   (~/.march/skills/git.md)：Git 提交规范
- api-style  (.march/skills/api-style.md)：本项目 API 风格约定
...
session 启动
    │
    ├─ 扫描 ~/.march/skills/ 和 .march/skills/
    │   └─ 同名时项目级覆盖用户级
    │
    ├─ 过滤 config.disable
    │
