...
Provider 相关的设置（API key、model、base_url）全部在 UI 里完成，Ma 自动持久化到用户级存储 `~/.ma/settings.db`，用户不需要手动编辑任何文件。
...
`~/.ma/settings.db` 由 Ma 独占管理，不是用户可编辑的格式。
...
| 网络重试次数 | 5 次 | 固定策略即可 |
| 本地 socket 路径 | `.ma/ma.sock` | 约定优于配置 |
