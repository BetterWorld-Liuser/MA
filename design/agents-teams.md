...
`name` 可省略，省略时用文件名（不含 `.md`）。`display_name` 可省略，省略时用 `name`。`avatar_color` 可省略，March 自动分配。
...
基础指令是所有角色共享的"地基"，确保无论什么角色都能正确使用工具、遵守安全规则。角色的 `system_prompt` 叠加在基础指令之上，定义角色特有的行为风格和专业方向。
