...
`ProviderType` 同时承担"来源通道类型"和"运行协议选择"两层语义。一个 `provider_type` 对应唯一的请求协议分支，运行时翻译层直接按 `ProviderType` 选择 `WireAdapter` 与端点分支，不再单独持久化 `WireFormat` 字段：
...
同样是"OpenAI 风格"的 server-side tool，也需要区分 `/responses` 和 `/chat/completions` 两套注入格式。这个分流属于 provider 兼容职责，不应上浮到 AgentContext。
...
-- 已知 provider（Anthropic/OpenAI/Gemini）的内置模型能力由 March 内置，不存表
-- 以下表存储所有用户已激活/接入的模型（包括已知 provider 中用户手动激活的内置模型）
...
CREATE TABLE model_server_tools (
    ...
    format           TEXT    NOT NULL,  -- 'anthropic' | 'openai_responses' | 'openai_chat' | 'gemini'
...
Provider 分为**官方 Provider** 和**自定义 Provider（中转站）**，两者的配置路径不同。
...
用户提供 base URL 和 API key，**供应商类型就已经决定了协议族**。不做协议自动探测，provider 页只负责让用户显式确认类型、填写凭据和测试连通性。
...
用户配置好 provider type、base URL 和 API key 后，March 直接按该类型对应的协议发一个最小请求：
...
请求体：`system: ""` + `user: "hi"` + `max_tokens: 1`。结论只有"可连通 / 不可连通"，**不做协议探测**。

### 候选模型发现（Provider 级，可选）

连通性确认后，可按 provider type 对应端点拉取候选模型列表，作为辅助信息：
...
候选模型列表只是"辅助选择"，不是自动探测的一部分；拿不到列表时，仍然允许用户手动填写 `model_id`。

### 能力确认（Model 级）

用户选择或手填 `model_id` 后，进入能力确认表单。不做自动能力探测，按来源类型走两条路径：
