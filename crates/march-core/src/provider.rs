...
fn backoff_delay(attempt: u32, base: Duration) -> Duration {
    let factor = 1u64 << attempt.min(4); // 最多 2^4 = 16 倍
...
impl ProviderClient {
        ...
        F: FnMut(ProviderProgressEvent) -> Result<()>,
    {
        ...
        // === 该 provider/model 已知不支持流式：直接走非流式（含重试）===
        ...
        // === 流式优先：最多重试 STREAM_MAX_RETRIES 次，之后 fallback 非流式 ===
