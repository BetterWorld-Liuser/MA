...
impl WatchedFileStore {
    ...
    pub fn remove_file(&self, path: impl AsRef<Path>, modified_by: ModifiedBy) -> Result<()> {
        let path = normalize_path_lossy(path.as_ref().to_path_buf());
        self.state
            .write()
            .expect("watch state poisoned")
        ...
        let watcher = notify::recommended_watcher(move |result: notify::Result<Event>| {
            if let Ok(event) = result {
                for path in event.paths {
                    let normalized = resolve_tracked_path(&state, &path)
                        .unwrap_or_else(|| normalize_path_lossy(path.clone()));
...
impl WatchState {
    fn mark_missing(&mut self, path: PathBuf, modified_by: ModifiedBy) {
        self.snapshots.insert(
            path.clone(),
            FileSnapshot::deleted(path, SystemTime::now(), modified_by),
        );
    }
}
...
fn normalize_path_lossy(path: PathBuf) -> PathBuf {
    path.canonicalize().map(clean_path).unwrap_or_else(|_| {
        if path.is_absolute() {
            clean_path(path)
        } else {
            clean_path(
...
fn resolve_tracked_path(state: &Arc<RwLock<WatchState>>, incoming_path: &Path) -> Option<PathBuf> {
    let direct = normalize_path_lossy(incoming_path.to_path_buf());
    let snapshots = &state.read().expect("watch state poisoned").snapshots;
    ...
    snapshots
        .keys()
        .find(|tracked| {
            tracked.file_name() == incoming_path.file_name()
