...
fn run_command_tool(available_shells: &[AvailableShell], working_directory: &Path) -> ToolDefinition {
    ToolDefinition {
        name: "run_command",
...
fn write_file_tool() -> ToolDefinition {
    ToolDefinition {
        name: "write_file",
        description: "Write the full content of a file in one operation.",
        parameters: vec![
            ToolParameter {
                name: "path",
                kind: "path",
...
fn insert_lines_tool() -> ToolDefinition {
    ToolDefinition {
        ...
        notes: vec![
            "Use insert_lines for additive edits that do not replace an existing region.".to_string(),
        ],
    }
}
...
fn delete_lines_tool() -> ToolDefinition {
    ToolDefinition {
        ...
        notes: vec![
            "Use delete_lines when the change is a pure removal and the line span is known.".to_string(),
        ],
    }
}
...
fn remove_note_tool() -> ToolDefinition {
    ToolDefinition {
        ...
        notes: vec![
            "Use remove_note once the stored state becomes stale, solved, or irrelevant.".to_string(),
        ],
    }
}
